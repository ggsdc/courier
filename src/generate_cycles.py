...
def create_single_cycles(id, points_set, cross_docking_points, distances):
    ...
    :param points_set:
    :type points_set:
    :param cross_docking_points:
    :type cross_docking_points:
    :param distances:
    :type distances:
    :return:
    :rtype:
    ...
    for cross in cross_docking_points:
        ...
        for origin in points_set:
            ...
            aux_distance = distances[
                (distances.originCode == cross) & (distances.destinationCode == origin)
            ]
...
def create_cycle_with_one_stop(id, points_set, cross_docking_points, times):
    ...
    for cross in cross_docking_points:
        ...
        while len(aux_set):
            ...
            stop = times_red.loc[
                times_red[times_red.originCode == cross]["Hours"].idxmin()
            ]["destinationCode"]
            time_aux = times_red.loc[
                times_red[times_red.originCode == cross]["Hours"].idxmin()
            ]["Hours"]
            times_aux = times_red[
                (times_red.originCode == stop)
                & (times_red.Hours + time_aux + 0.25 <= 6)
            ]
            ...
            times_red = times_red[
                (times_red.originCode != stop) & (times_red.destinationCode != stop)
            ]
