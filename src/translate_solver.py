    ...
    for ivar in set(var_names_index.keys()):
        ...
        for i in range(0, len(var_keys_aux)):
            ...
            if k == var_names_index[ivar][0]:
                ...
                for p in range(0, k):
                    position.append(
                        [j for j, x in enumerate(var_keys_aux[i][len(ivar): len(var_keys_aux[i])]) if x == "_"][p] + 1)
                ...
                if k > 1:
                    ...
                    if var_names_index[ivar][1] == 'integer':
                        index[j] = int(
                            var_keys_aux[i][len(ivar): len(var_keys_aux[i])][(position[j] + 1):(position[j + 1] - 2)])
                    elif var_names_index[ivar][1] == 'float':
                        index[j] = float(
                            var_keys_aux[i][len(ivar): len(var_keys_aux[i])][(position[j] + 1):(position[j + 1] - 2)])
                    elif var_names_index[ivar][1] == 'string':
                        index[j] = var_keys_aux[i][len(ivar): len(var_keys_aux[i])][(position[j] + 2):(position[j + 1] - 3)]

                    for j in range(1, len(position) - 1):
                        if var_names_index[ivar][j + 1] == 'integer':
                            index[j] = int(
                                var_keys_aux[i][len(ivar): len(var_keys_aux[i])][(position[j] + 1):(position[j + 1] - 3)])
                    ...
                    j = len(position) - 1
                    if var_names_index[ivar][j + 1] == 'integer':
                        index[j] = int(var_keys_aux[i][len(ivar): len(var_keys_aux[i])][
                                       (position[-1]):(len(var_keys_aux[i][len(ivar): len(var_keys_aux[i])]) - 1)])
                    elif var_names_index[ivar][j + 1] == 'float':
                        index[j] = float(var_keys_aux[i][len(ivar): len(var_keys_aux[i])][
                                         (position[-1]):(len(var_keys_aux[i][len(ivar): len(var_keys_aux[i])]) - 1)])
                    elif var_names_index[ivar][j + 1] == 'string':
                        index[j] = var_keys_aux[i][len(ivar): len(var_keys_aux[i])][
                                   (position[-1] + 1):(len(var_keys_aux[i][len(ivar): len(var_keys_aux[i])]) - 1)]

                    output_temp[tuple(index)] = var_names_index[ivar][j + 2] * var_values[var_keys_aux[i]]
                elif k == 1:
                    if var_names_index[ivar][1] == 'integer':
                        index = int(var_keys_aux[i][(len(ivar) + 1): len(var_keys_aux[i])])
                        output_temp[index] = var_values[var_keys_aux[i]]
                    elif var_names_index[ivar][1] == 'float':
                        index = float(var_keys_aux[i][(len(ivar) + 1): len(var_keys_aux[i])])
                        output_temp[index] = var_values[var_keys_aux[i]]
                    elif var_names_index[ivar][1] == 'string':
                        index = var_keys_aux[i][(len(ivar) + 1): len(var_keys_aux[i])]

                    output_temp[index] = var_names_index[ivar][-1] * var_values[var_keys_aux[i]]
                elif k == 0:
                    output_temp[var_keys_aux[i]] = var_names_index[ivar][-1] * var_values[var_keys_aux[i]]
            else:
