...
"""
# & c:/Users/bob_guille/Desktop/proyectos/i-d-courier/courier/venv/Scripts/python.exe
...
def main():
    ...
    vehicles_data, points_data, demand_data = rd.read_data(
        vehicles_path, points_path, demand_path
    )
    ...
    for cross in cross_docking_points:
        ...
        temp_paths, path_idx = sg.simple_path_generation(
            cross, points_dict, times_dict, distance_dict, demand_dicts, path_idx
        )
        t2 = datetime.datetime.now()
        ...
        t1 = datetime.datetime.now()
        # Get complex paths
        temp_paths, path_idx = sg.complex_path_generation(
            cross,
            cross_docking_points,
            temp_paths,
            points_dict,
            times_dict,
            distance_dict,
        ...
        temp_cycles, selected_paths, selected_paths_2, cycle_idx = sg.cycle_generation(
            cross, aux_list, points_dict, cycle_idx
        )
        t2 = datetime.datetime.now()
        ...
        print(
            "Size of cycles: ",
            getsizeof(cycle_list) / 1024 / 1024,
            ". Size of itineraries: ",
            getsizeof(itinerary_list) / 1024 / 1024,
        )
        del (
            temp_paths,
            temp_cycles,
    ...
    # Objective function
    v_objective_function = lp.LpVariable(
        name="v_objective_function", lowBound=0, cat="Continuous"
    )
    v_number_vehicles = lp.LpVariable(
        name="v_number_vehicles", lowBound=0, cat="Continuous"
    )

    # Cycle variable - number of vehicles per cycle
    v_vehicles_cycle = lp.LpVariable.dicts(
        "Vehicles", cycle_list, lowBound=0, cat="Integer"
    )

    # Flow variable - amount of commodity k moved in itinerary i
    v_flow_itinerary = lp.LpVariable.dicts(
        "Flow",
        (
            (k, i)
    ...
    model += (
        lp.lpSum(
            v_vehicles_cycle[c] * c.length * vehicles_dict[str(c.vehicle)]["cost"]
            for c in cycle_list
        )
    ...
    model += (
        lp.lpSum(v_vehicles_cycle[c] for c in cycle_list) == v_number_vehicles,
        "RQ00.2",
    )
    ...
    for a in arcs_list:
        ...
        model += (
            ...
            <= lp.lpSum(
                v_vehicles_cycle[c]
                * vehicles_dict[str(c.vehicle)]["capacity"]
                * p_omega[
                    (
                        c,
                        a,
    ...
    solution = tr.translate_solver(
        var_names_index, var_values, cycle_list, itinerary_list, commodities
    )
    ...
    out3.to_csv(
        path_or_buf="..\\data\\vehicles.csv", sep=";", index=False, encoding="utf-8"
    )
